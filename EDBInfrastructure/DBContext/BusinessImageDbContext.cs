                ...
                using (var tr = _conn.BeginTransaction())
                {
                    try
                    {
                        using (SqlCommand command = new SqlCommand())
                        {
                            ...
                            command.Parameters.Add("@userid", SqlDbType.Int).Value = businessImageDetails.UserId;                            
                            command.Parameters.Add("@businessid", SqlDbType.Int).Value = businessImageDetails.BusinessId;
                ...
                using (var tr = _conn.BeginTransaction())
                {
                    try
                    {
                        using (SqlCommand command = new SqlCommand())
                        {
                            ...
                            //command.Parameters.Add("@userid", SqlDbType.BigInt).Value = businessImageDetails.UserId;
                            ...
                            //command.Parameters.Add("@businessid", SqlDbType.BigInt).Value = businessImageDetails.BusinessId;                        
                            int result = command.ExecuteNonQuery();
                            if (1 != result)
                            {
                                throw (new Exception("Failed to update record in database. Unknown error"));
